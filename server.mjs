...
async function localBrief(task){const r=await fetch(`${localUrl}/api/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model:localModel,stream:false,options:{temperature:.35,num_predict:400},messages:[{role:'system',content:briefPolicy},{role:'user',content:`Task: ${task}`}]})});const j=await r.json();if(!r.ok||!j.message?.content)throw Error('Local model unavailable');return j.message.content}
